...
fn test_preset_themes_include_popular_themes() {
    ...
    assert!(theme_names.contains(&"Ocean Breeze".to_string()), "Should include Ocean Breeze");
    assert!(theme_names.contains(&"Forest Harmony".to_string()), "Should include Forest Harmony");
    assert!(theme_names.contains(&"Sunset Glow".to_string()), "Should include Sunset Glow");
}
...
fn test_preset_themes_have_base_themes() {
    ...
    for (_, config) in themes {
        assert!(!config.base_theme.is_empty(), "Each preset should have a base theme");
    }
}
...
fn test_preset_themes_no_duplicates() {
    ...
    let unique_count = theme_ids.iter().collect::<std::collections::HashSet<_>>().len();
...
fn test_preset_themes_have_metadata() {
    ...
    for (id, config) in themes {
        assert!(config.metadata.is_some(), "Theme '{}' should have metadata", id);
    }
}
...
fn test_ocean_breeze_theme_exists() {
    ...
    if let Some((_, config)) = ocean_breeze {
        assert_eq!(config.base_theme, "aqua", "Ocean Breeze should use aqua base theme");
    }
}
...
fn test_forest_harmony_theme_exists() {
    ...
    if let Some((_, config)) = forest {
        assert_eq!(config.base_theme, "forest", "Forest Harmony should use forest base theme");
    }
}
...
fn test_midnight_sky_theme_exists() {
    ...
    if let Some((_, config)) = midnight {
        assert_eq!(config.base_theme, "night", "Midnight Sky should use night base theme");
    }
}
