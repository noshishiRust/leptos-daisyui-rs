...
fn test_export_includes_base_theme() {
    ...
    assert!(
        fields.contains(&"base_theme"),
        "Export should include base_theme"
    );
}
...
fn test_export_includes_overrides() {
    let fields = ["base_theme", "overrides", "metadata"];
    assert!(
        fields.contains(&"overrides"),
        "Export should include overrides"
    );
}
...
fn test_default_filename_extension() {
    ...
    assert!(
        ...
        "Export filename should end with .json"
    );
}
...
fn test_import_expects_json_format() {
    ...
    assert!(
        valid_extensions.contains(&"json"),
        "Import should accept JSON files"
    );
}
...
fn test_export_import_operations() {
    ...
    assert!(
        operations.contains(&"copy_to_clipboard"),
        "Should support clipboard copy"
    );
}
...
fn test_status_messages_for_operations() {
    ...
    assert!(
        success_message.contains("successfully"),
        "Success message should indicate success"
    );
    assert!(
        error_prefix.contains("failed"),
