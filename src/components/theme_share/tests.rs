...
fn test_url_separator_logic() {
    ...
    let separator_without = if url_without_params.contains('?') {
        "&"
    } else {
        "?"
    };
    let separator_with = if url_with_params.contains('?') {
        "&"
    } else {
        "?"
    };
    ...
    assert_eq!(
        separator_with, "&",
        "Should use & for additional parameters"
    );
}
...
fn test_share_url_format() {
    ...
    assert!(
        url.contains("theme="),
        "Share URL should contain theme parameter"
    );
}
...
fn test_base64_url_safe_characters() {
    ...
    assert!(
        !url_safe_chars.contains(&'+'),
        "Should not use + (not URL-safe)"
    );
    assert!(
        !url_safe_chars.contains(&'/'),
        "Should not use / (not URL-safe)"
...
fn test_share_operations() {
    ...
    assert!(
        operations.contains(&"generate_url"),
        "Should generate share URL"
    );
    assert!(
        operations.contains(&"copy_url"),
        "Should copy URL to clipboard"
    );
    assert!(
        operations.contains(&"load_from_url"),
        "Should load theme from URL"
    );
}
...
fn test_url_encoding_preserves_json() {
    ...
    assert!(
        encoded_length > 0,
        "Encoded URL should have non-zero length"
    );
    assert!(
        encoded_length >= json.len(),
        "Base64 encoding typically increases size"
    );
}
