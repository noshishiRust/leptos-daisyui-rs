...
fn test_border_width_increments() {
    ...
    assert_eq!(expected_steps, 17, "Should have 17 possible border width values");
}
...
fn test_spacing_scale_range() {
    ...
    assert!(min_scale < default_scale, "Min spacing should be less than default");
    assert!(max_scale > default_scale, "Max spacing should be greater than default");
...
fn test_border_radius_range() {
    ...
    assert!(max_radius >= 2.0, "Max border radius should be at least 2.0");
    assert!(default_radius > 0.0, "Default border radius should be positive");
}
...
fn test_border_customization_categories() {
    ...
    assert_eq!(categories.len(), 3, "Should have 3 border customization categories");
    assert!(categories.contains(&"border_width"), "Should include border width");
    assert!(categories.contains(&"border_radius"), "Should include border radius");
    assert!(categories.contains(&"spacing_scale"), "Should include spacing scale");
}
