...
fn initialize_generated_directory(project: &crate::project::ProjectStructure) -> Result<()> {
    ...
    if generated_dir.exists() {
        ...
        // Check if mod.rs exists, if not create it
...
"#;
            ...
            println!("{} Created mod.rs", "✓".green());
        }

        // Check if utils.rs exists, if not create it
        let utils_rs = generated_dir.join("utils.rs");
        if !utils_rs.exists() {
            create_utils_file(&utils_rs)?;
            println!("{} Added utils.rs (merge_classes utilities)", "✓".green());
        }
